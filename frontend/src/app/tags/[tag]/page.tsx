...
export default async function TagPage({ params }: { params: Promise<{ tag: string }> }) {
  ...
  try {
    const res = await fetch(apiUrl(`/api/v1/tags/${encodeURIComponent(tag)}?limit=50&offset=0`), { cache: "no-store" });
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] px-5 py-10">
      <div className="mx-auto max-w-3xl">
        <div className="mb-6">
          <h1 className="text-3xl font-semibold tracking-tight text-neutral-900">标签：{decodeURIComponent(tag)}</h1>
          <p className="mt-1 text-sm text-neutral-600">只展示公开可见文章</p>
        </div>

        {error ? (
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900">{error}</div>
        ) : posts.length === 0 ? (
          <div className="rounded-2xl border border-black/10 bg-white/70 p-6 text-sm text-neutral-700">暂无文章。</div>
        ) : (
